...
#ifndef _WIN32 
static void TGALoader::swap_16bit_word(uint16_t *word)
{
...
// an image
TGALoader::tgaInfo * TGALoader::tgaLoad(char *filename)
{
	FILE *file;
	tgaInfo *info;
	int mode, total;
...
#pragma warning(default: 4996)
	...
	if (ferror(file)) 
	{
		...
		mLogger->LogError("error reading texture file");
		fclose(file);
		return(info);
	}
...
void TGALoader::tgaLoadHeader(FILE *file, tgaInfo *info)
{
...
// directly
void TGALoader::tgaLoadImageData(FILE *file, tgaInfo *info) 
{
...
void TGALoader::tgaRGBtoGreyscale(tgaInfo *info) 
{
	int mode, i, j;
...
int TGALoader::tgaGrabScreenSeries(char *filename, int x, int y, int w, int h) 
{
...
int TGALoader::tgaSave(char            *filename,
	short int        width,
	short int        height,
...
int TGALoader::tgaSaveSeries(char        *filename,
	short int        width,
	short int        height,
	unsigned char    pixelDepth,
	unsigned char    *imageData) {
...
void TGALoader::tgaDestroy(tgaInfo *info) 
{
...
void TGALoader::loadTexture(std::string Filename)
{
	...
	glGenTextures(1, &TextureId);	// Es wird eine ID für die Textur generiert. texture ist eine globale var

	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glBindTexture(GL_TEXTURE_2D, TextureId);	// alle befehle die nachher kommen, sollen mit dieser ID verknüpft werden
...
void TGALoader::reportGLError(const char * msg)
{
	GLenum errCode;
